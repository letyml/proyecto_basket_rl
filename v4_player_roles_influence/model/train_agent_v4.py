...
from BasketballBoardCLI import BasketballBoardCLIV4
...
mlflow.set_experiment("Basketball RL Model V4")
        ...
        for ep_idx, episodio in enumerate(callback.episode_data):
            for step in episodio:
                row = {
                    ...
                    "prob_info": step.get("prob_info", None),
                    "player_name": step.get("player_name", "N/A")
                }
...
with mlflow.start_run() as run:
    ...
    run_dir = os.path.join(results_path, f"model_v4_{timestamp}")
    os.makedirs(run_dir, exist_ok=True)
    model_path = os.path.join(run_dir, "train_v4")

    mlflow.set_tag("run_name", f"modelo_v4_{timestamp}")
    ...
    version_info_path = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "v4_player_roles_influence.txt"))
    if os.path.exists(version_info_path):
        mlflow.log_artifact(version_info_path, artifact_path="metadata")
    else:
        print("No se encontró v4_player_roles_influence.txt en la ruta esperada:", version_info_path)


    env = BasketballBoardCLIV4()
    model = PPO(
        "MlpPolicy",
        env,
