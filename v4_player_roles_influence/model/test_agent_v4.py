...
from BasketballBoardVisual import BasketballBoardVisualV4
...
def play_episode(csv_path, episodio=0, fps=5, enable_countdown=False, visual=True):
    ...
    env = BasketballBoardVisualV4(render_mode="human")
    ...
    for _, row in df_ep.iterrows():
        ...
        player_name = row["player_name"] if "player_name" in row else "N/A"

        print(f"Jugador: {player_name} | Acción={row['action']} | Jugador={env.current_player_idx} | Recompensa={row['reward']} | Balón={env.ball_pos} | Tiro: {row['shot']} | "
              f"Defensa: {row['prob_info']} | "
...
if __name__ == "__main__":
    nombre_modelo = input("Nombre del modelo (ej. model_v4_YYYYMMDD_HHMMSS): ").strip()
