...
from BasketballBoardCLI import BasketballBoardCLIV3
...
mlflow.set_experiment("Basketball RL Model V3")
...
with mlflow.start_run() as run:
    ...
    run_dir = os.path.join(results_path, f"model_v3_{timestamp}")
    os.makedirs(run_dir, exist_ok=True)
    model_path = os.path.join(run_dir, "train_v3")

    mlflow.set_tag("run_name", f"modelo_v3_{timestamp}")
    ...
    version_info_path = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "v3_defense_interception.txt"))
    if os.path.exists(version_info_path):
        mlflow.log_artifact(version_info_path, artifact_path="metadata")
    else:
        print("No se encontró v3_defense_interception.txt en la ruta esperada:", version_info_path)
